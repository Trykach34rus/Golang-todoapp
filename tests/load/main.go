...
func setupUsers(base string, n int) ([]int, error) {
	ids := make([]int, 0, n)
	for i := 0; i < n; i++ {
...
func teardownTasks(base string, m map[int][]int) {
	for _, tasks := range m {
		for _, tid := range tasks {
			doJSON(http.MethodDelete, fmt.Sprintf("%s/tasks/%d", base, tid), nil)
		}
	}
}

func teardownUsers(base string, ids []int) {
	for _, id := range ids {
		doJSON(http.MethodDelete, fmt.Sprintf("%s/users/%d", base, id), nil)
	}
}
...
func mixedWorker(
	...
	defer wg.Done()
	myTasks := make([]int, len(tasksByUser[ID]))
	copy(myTasks, tasksByUser[ID])
	for {
		select {
		case <-stop:
			return
		...
		for i := 0; i < *mixedReadCount; i++ {
			...
			{
				url := fmt.Sprintf("%s/tasks?user_id=%d&limit=10&offset=%d",
					base, ID, rand.Intn(maxInt(1, len(myTasks)-10)))
				start := time.Now()
				resp, _, err := doJSON(http.MethodGet, url, nil)
				sListSelf.Record(time.Since(start), err == nil && resp.StatusCode == 200)
			}
		...
		for i := 0; i < *mixedWriteCount; i++ {
			...
			// POST
			{
				start := time.Now()
				resp, body, err := doJSON(http.MethodPost, base+"/tasks", CreateTaskRequest{
					Title:        fmt.Sprintf("Mixed %s [u:%d]", randomString(6), ID),
					Description:  fmt.Sprintf("Mixed load task user %d", ID),
					AuthorUserID: ID,
				})
...
func runPhase(
	...
	base string, userIDs []int, tasksByUser map[int][]int,
	...
	for i := 0; i < conc; i++ {
		...
		wg.Add(1)
		go func(u int) { launch(stop, &wg, u, &iters) }(uid)
	}
	time.Sleep(dur)
	close(stop)
	wg.Wait()
